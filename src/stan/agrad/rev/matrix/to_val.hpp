#ifndef __STAN__AGRAD__REV__MATRIX__TO_VAL_HPP__
#define __STAN__AGRAD__REV__MATRIX__TO_VAL_HPP__
...
namespace stan {
  namespace agrad {
    ...
    inline const Eigen::Matrix<double,R,C> &to_val(const Eigen::Matrix<double,R,C> &M) {
      return M;
    }
     ...
     **/
    template<int R,int C>
    inline Eigen::Matrix<double,R,C> to_val(const Eigen::Matrix<var,R,C> &M) {
      size_t i,j;
